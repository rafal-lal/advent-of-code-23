	...
	outer:
		for len(fifo) != 0 {
			...
			for _, target := range modules[modName].GetTargets() {
				allSet := true
				for k, v := range modsToCheck {
					if target == k {
						if modsToCheck[k] == -1 && signal == 1 {
							modsToCheck[k] = buttonCnt
						}
					}
					if v == -1 {
						allSet = false
				...
				if allSet {
					fmt.Printf("Button presses: %d\n",
						utils.LCM(modsToCheck["kl"], modsToCheck["vm"], modsToCheck["kv"], modsToCheck["vb"]))
					return
				}
